  ...
  return (
    <div style={styles.appShell}>
      ...
      <div style={styles.mapStage}>
        ...
        <div style={styles.caseFileOverlay}>
          <CaseFile state={state} />
        </div>

        {isChamber && (
          <ChamberActionBar scene={scene} goTo={goTo} />
        )}
        ...
        {isHub && (
          <HubMapOverlay scene={scene} goTo={goTo} updateState={updateState} />
        )}
      </div>
...
function HubMapOverlay({ scene, goTo, updateState }) {
    ...
    <div style={{ position: "absolute", inset: 0, zIndex: 15 }}>
      ...
      >
        ...
        {scene.actions.map((action) => {
          ...
          return (
            ...
            >
              {isPending && (
                <circle cx={x} cy={y} r={24} fill="none" stroke="#4A9FD4" strokeWidth={1.5} opacity={0.5} />
              )}
              <circle cx={x} cy={y} r={14} fill={fillColor} stroke={ringColor} strokeWidth={2.5} />
              {done ? (
                <text x={x} y={y + 5} textAnchor="middle" fontSize="11" fill="#6A9A50" fontWeight="bold" style={{ pointerEvents: "none" }}>✓</text>
              ) : (
                <circle cx={x} cy={y} r={4} fill={ringColor} opacity={blocked ? 0.25 : 0.85} style={{ pointerEvents: "none" }} />
              )}
      ...
      <div style={{
        ...
        <div style={{
          ...
          {slotsLeft > 0
            ? `${slotsLeft} stop${slotsLeft !== 1 ? "s" : ""} remaining`
            : "All stops taken"}
        </div>
        <button
...
function ChamberActionBar({ scene, goTo }) {
  return (
    <div style={{
      position: "absolute", top: "50%", left: 18,
      transform: "translateY(-50%)",
